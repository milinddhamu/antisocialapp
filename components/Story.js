...
function Story({userId,img, username,active}) {
    ...
    return (
        <div>
            {img && !active && (
                <>
                    <img onClick={handleClick} className='h-14 w-14 rounded-full p-[1.5px] border-violet-500 border-2 object-contain cursor-pointer hover:scale-110 transition transform duration-200 ease-out' src={img} alt="" />
               ...
               active && userId!==session?.user?.uid && (
                   <>
                    <div onClick={()=>setOpen(true)} className="relative">
                     ...
                     <PlusCircleIcon className="h-6 text-cyan-600 absolute left-[2.12rem] cursor-pointer bottom-3 bg-white rounded-full "/>
                    <p className='text-xs w-14 truncate text-center'>{username}</p>   
                   </div>
                   </>
            )
